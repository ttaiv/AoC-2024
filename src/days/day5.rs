...
pub fn solve() -> (u32, u32) {
    ...
    let part1_result: u32 = page_updates
        .iter()
        .filter(|update| is_in_correct_order(update, &successors))
        .map(|update| update[update.len() / 2])
        .sum::<u32>();

    let part2_result: u32 = page_updates
        .iter()
        .filter(|update| !is_in_correct_order(update, &successors))
...
fn subgraph_dfs(adj_list: &HashMap<u32, HashSet<u32>>, nodes_to_consider: &[u32]) -> Vec<u32> {
    ...
    fn dfs(
        ...
        nodes_to_consider: &[u32],
        visited: &mut HashSet<u32>,
        finished_order: &mut Vec<u32>,
    ) {
...
fn parse_input() -> (HashMap<u32, HashSet<u32>>, Vec<Vec<u32>>) {
    let data = fs::read_to_string("inputs/day5.txt").expect("Input file should be available");
